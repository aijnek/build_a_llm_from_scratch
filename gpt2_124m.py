...
if __name__ == "__main__":
    ...
    print(f"Transformer block parameters: {trf_params:,}")

    trf = model.trf_blocks[0]
    att_params = sum(p.numel() for p in trf.att.parameters())
    print(f"Attention parameters in a single trf block: {att_params:,}")

    ff_params = sum(p.numel() for p in trf.ff.parameters())
    print(f"Feed-forward parameters in a single trf block: {ff_params:,}")
